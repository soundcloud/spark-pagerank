...
class PlainPageRankTest
  ...
  private def execTest(
    ...
    // TODO(jd): use standard graph building methods

    val (srcNodes, dstNodes) = input.map(x => (x._1, x._2)).unzip
    val n = (srcNodes ++ dstNodes).distinct.size
    val prior = 1.0 / n

    val rdd = sc.parallelize(expandEdgeTuples(input))
    val edges = normalizeOutEdgeWeightsRDD(rdd)
      .map { e =>
        (e.srcId, PlainPageRank.Edge(e.dstId, e.attr))
      }
      .persist(StorageLevel.MEMORY_ONLY)
    val vertexValues = edges
      .flatMap { case (srcId, edge) =>
        Seq(srcId, edge.dstId)
      }
      .distinct()
      .map((_, prior))
    val vertices = PlainPageRank.buildVerticesFromValues(edges, vertexValues)
      .persist(StorageLevel.MEMORY_ONLY)

    val actual = PlainPageRank
      .run(
        edges,
        vertices,
        teleportProb,
    ...
    def squaredError(actual: Seq[(Long, PlainPageRank.VertexMetadata)], expected: Seq[(Int, Double)]): Double = {
